...
def sdw_mwf(u, vad, target_pos, source_pos, fs):
    ...
    # Initialize overlap-save buffers before the loop
    ...
    for m in range(tot_frames):
        ...
        # 1. Multiplicación en frecuencia (convolución circular)
        D_circ = np.einsum('fm, mf->f', w_fixed, U_fram_rfft)
        D_n_circ = np.einsum('fmn, mf->fn', C_block, U_fram_rfft)

        # 2. Transformar al tiempo
        d_time_2L = np.fft.irfft(D_circ, n=2 * L)
        dn_time_2L = np.fft.irfft(D_n_circ, n=2 * L, axis=0)

        # 3. Descartar la basura (wrap-around)
        d_valid = d_time_2L[L:] 
        dn_valid = dn_time_2L[L:, :]

        # 4. Reconstruir bloques limpios de 2L con el frame anterior
        d_2L_true = np.concatenate((y0_prev, d_valid))
        dn_2L_true = np.concatenate((yn_prev, dn_valid), axis=0)

        # 5. Actualizar buffers de overlap-save
        y0_prev = d_valid.copy()
        yn_prev = dn_valid.copy()

        # 6. Volver a frecuencia (ahora sin artefactos)
        D_true = np.fft.rfft(d_2L_true)
        D_n_true = np.fft.rfft(dn_2L_true, axis=0)

        # 7. Combinar referencia de habla y ruido: shape (F, M)
        D_y_true = np.column_stack((D_true, D_n_true)) 
        ...
        post_block[m * L : m * L + L] = dn_valid[:, 0]

        # --- PROCESS ADAPTIVE BRANCH ---
        # Usamos D_y_true (la versión limpia)
