...
def sdw_mwf(u, vad, target_pos, source_pos, fs):
    ...
    # Contadores para el WARM-UP
    min_warmup_frames = 15 # Esperar a ver 15 frames de voz y 15 de ruido antes de adaptar
    speech_frames_seen = 0
    noise_frames_seen = 0

# ... [tu inicialización anterior] ...

    # Nuevos buffers para el Overlap-Save de las referencias
    y0_prev = np.zeros(L, dtype=np.float64)
    yn_prev = np.zeros((L, M - 1), dtype=np.float64)
    ...
    for m in range(tot_frames):
        ...
        # --- PROCESS FIXED BRANCH (Preprocesador Espacial) ---
        # Beamformer fijo
        D = np.einsum('fm, mf->f', w_fixed, U_fram_rfft)
        y0_2L = np.fft.irfft(D, n=2 * L)
        y0_valid = y0_2L[L:] # Nos quedamos solo con la convolución lineal válida
        
        # Matriz de bloqueo
        D_n = np.einsum('fmn, mf->fn', C_block, U_fram_rfft)
        yn_2L = np.fft.irfft(D_n, n=2 * L, axis=0)
        yn_valid = yn_2L[L:, :] # Nos quedamos solo con la convolución lineal válida

        # --- RECONSTRUIR BLOQUES LIMPIOS PARA EL FILTRO ADAPTATIVO ---
        # Concatenamos la parte válida del frame anterior con el actual
        y0_2L_true = np.concatenate((y0_prev, y0_valid))
        yn_2L_true = np.concatenate((yn_prev, yn_valid), axis=0)

        # Guardamos el frame actual para la próxima iteración
        y0_prev = y0_valid.copy()
        yn_prev = yn_valid.copy()

        # Volvemos al dominio de la frecuencia (AHORA SÍ SIN ARTEFACTOS)
        Y0_f = np.fft.rfft(y0_2L_true)
        Yn_f = np.fft.rfft(yn_2L_true, axis=0)
        
        # Combinamos referencia de habla y de ruido: shape (F, M)
        D_y_true = np.column_stack((Y0_f, Yn_f))

        # --- APPLY DELAY FIXED BRANCH ---
        # El objetivo (target) es la referencia de habla retrasada
        d_combined = np.concatenate((history_buffer, y0_valid))
        start_idx = L - delay
        end_idx = 2 * L - delay
        d_delayed_valid = d_combined[start_idx : end_idx]
        
        history_buffer = y0_valid.copy()
        z_fixed_branch[m * L : m * L + L] = y0_valid

        # --- PROCCES ADAPTIVE BRANCH ---
        # Usamos D_y_true que está limpio de aliasing circular
        noise = np.einsum('fm,fm->f', D_y_true, w_adaptive)
        ...
        # Zeropad y FFT para el gradiente (esto estaba perfecto en tu código original)
        e_2L = np.concatenate((np.zeros(L, dtype=np.float64), e))
        e_undeline = np.fft.rfft(e_2L)
        
        # --- UPDATE CORRELATION MATRICES ---
        # Calculamos la energía espacial con las referencias corregidas
        Q_instant = np.einsum('fm,fn->fmn', D_y_true.conj(), D_y_true)
        ...
        if vad_status: 
            ...
            if speech_frames_seen >= min_warmup_frames and noise_frames_seen >= min_warmup_frames:
                ...
                # Calculamos el gradiente con D_y_true
                gradient = np.einsum('fm, f -> fm', D_y_true.conj(), e_undeline) - r_2NL
                
                gradient_update = np.linalg.solve(Q_mix, gradient)

                w_adaptive = w_adaptive + rho * (1 - Lambda) * 0.5 * gradient_update
                
    print(f"Max output value : {np.max(z)}")
    print(f"Contien NaNs: {np.isnan(z).any()}")
