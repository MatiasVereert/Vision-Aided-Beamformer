...
if __name__ == "__main__":
    ...
    # Calculate X and Y coordinates for the Uniform Circular Array (UCA)
    # Z coordinates remain 0 assuming a flat 2D planar array geometry
    x = radius * np.cos(angles)
    y = radius * np.sin(angles)
    z = np.zeros(M)

    # Stack the coordinates into a single array of shape (M, 3)
    mic_coords_ideal = np.column_stack([x, y, z])

    # Define the absolute center position of the array in the simulated room
    array_center = np.array([1.25, 2.0, 1.25])

    # Translate the array coordinates to the desired room center position
    # Note: The UCA is already centered at (0,0,0), so we just add the offset
