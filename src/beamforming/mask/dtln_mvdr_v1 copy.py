...
# =====================================================================
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, min_loading=1e-6, lamda=0.99):
    K, T, M = X_stft.shape  
    ...
    # --- COHERENCE-BASED POST-FILTER INIT ---
    lamda_pf = 0.68  # Forgetting factor for spatial coherence PSDs
    mu = 1.1         # Overestimation factor for DOA-independent estimator
    G_min = 0.1      # Gain floor to mask musical noise
    
    Phi_XX_align = np.tile(np.eye(M, dtype=np.complex128) * 1e-6, (K, 1, 1))
    G_pf_smooth = np.ones(K, dtype=np.float64) # Memory for temporal gain smoothing
    # ----------------------------------------
    ...
    for m in range(T):
        ...
        # The denominator is always real. Force np.real to prevent imaginary residuals.
        weights = weights_nom / (np.real(weights_den[:, np.newaxis]) + 1e-10)

        # 1. Linear MVDR Output
        Y_mvdr = np.einsum("fm,fm->f", weights.conj(), X_frame)
        
        # =========================================================
        # 2. COHERENCE-BASED POST-FILTERING (Schwarz et al. 2015)
        # =========================================================
        
        # Align inputs using the MVDR steering vector's phase
        d_phase = d / (np.abs(d) + 1e-15)
        X_aligned = X_frame * d_phase.conj()
        
        # Update spatial coherence matrix of aligned signals
        R_align = np.einsum("fm,fn->fmn", X_aligned, X_aligned.conj())
        Phi_XX_align = lamda_pf * Phi_XX_align + (1 - lamda_pf) * R_align
        
        # Extract Auto-power (A) and Cross-power (C)
        auto_power = np.real(np.trace(Phi_XX_align, axis1=1, axis2=2)) / M
        total_power = np.real(np.sum(Phi_XX_align, axis=(1, 2)))
        
        # Cross power is the mean of the off-diagonal elements
        cross_power = (total_power - auto_power * M) / (M * (M - 1))
        
        # Prevent division by zero mathematically
        auto_power = np.maximum(auto_power, 1e-15)
        cross_power = np.maximum(cross_power, 1e-15)
        
        # Estimate Diffuse ratio: 1 / (1 + CDR) = (A - C) / A = 1 - C/A
        diffuse_ratio = np.clip(1.0 - (cross_power / auto_power), 0.0, 1.0)
        
        # Compute Wiener Post-Filter Gain
        G_pf = np.maximum(1.0 - mu * diffuse_ratio, G_min)
        
        # Smooth the gain over time to prevent musical noise
        G_pf_smooth = 0.8 * G_pf_smooth + 0.2 * G_pf
            
        # Apply the final spatial gain to the MVDR output
        Y_stft[:, m] = Y_mvdr * G_pf_smooth
...
if __name__ == "__main__":
    ...
    if M <= 1:
        ...
        x = np.array([0.0, d_max])
    else:
        # Calculate the required expansion ratio
        R = d_max / d_min
        
        # Build the polynomial: b^(M-2) + b^(M-3) + ... + b + (1 - R) = 0
        coeffs = np.ones(M - 1)
        coeffs[-1] = 1.0 - R
        ...
        # Extract the real, positive root to use as the expansion base
        real_roots = roots[np.isclose(np.imag(roots), 0.0)].real
        b = real_roots[real_roots > 0.0][0]
        
        indices = np.arange(M)
        
        # Generate the spatial distribution
        # Fallback to linear spacing if d_max exactly implies uniform spacing
        if np.isclose(b, 1.0):
            x = indices * d_min
        else:
            x = d_min * (b**indices - 1.0) / (b - 1.0)
