...
def GEV_recursive_mask_based(X_stft, mask_s, mask_n, min_loading=1e-6, lamda=0.92):
    # Ensure scipy.linalg is imported at the top of your script
    K, T, M = X_stft.shape  
    
    Y_stft = np.zeros((K, T), dtype=np.complex128)
    
    # Initialize PSD matrices with small diagonal loading for stability
    Phi_NN = np.tile(np.eye(M, dtype=np.complex128) * 1e-6, (K, 1, 1))
    Phi_XX = np.tile(np.eye(M, dtype=np.complex128) * 1e-6, (K, 1, 1))
    
    # Memory for the previous valid GEV filter
    F_gev_prev = np.ones((K, M), dtype=np.complex128) / np.sqrt(M)
    
    for m in range(T):
        print(f"\rProcessing frame {m+1} of {T}", end="")
        X_frame = X_stft[:, m, :]
        ...
        # Recursive update of PSD matrices
        Phi_XX = lamda * Phi_XX + (1 - lamda) * (m_s_frame * R_instant)
        Phi_NN = lamda * Phi_NN + (1 - lamda) * (m_n_frame * R_instant)

        # Adaptive diagonal loading to ensure Phi_NN is positive definite
        ...
        F_gev = np.zeros((K, M), dtype=np.complex128)
        
        # Solve the Generalized Eigenvalue Problem per frequency bin
        # We avoid np.linalg.inv(Phi_NN) to keep the numerical stability advantage of GEV
        for f in range(K):
            try:
                # eigh(A, B) solves A v = lambda B v
                # It returns eigenvalues in ascending order, so the last is the principal
                evals, evecs = spla.eigh(Phi_XX[f], Phi_NN_stable[f])
                F_gev[f, :] = evecs[:, -1]
            except np.linalg.LinAlgError:
                # Fallback to the previous frame's filter if decomposition fails
                F_gev[f, :] = F_gev_prev[f, :]
        
        # Save filter for future fallback
        F_gev_prev = F_gev.copy()
        
        # --- Apply BAN Post-Filter ---
        # Compute Phi_NN * F_gev
        Phi_NN_F = np.einsum("fmn,fn->fm", Phi_NN_stable, F_gev)
        
        # Numerator: sqrt( F^H * Phi_NN * Phi_NN * F / M )
        # Since Phi_NN is Hermitian, F^H Phi_NN Phi_NN F is the squared L2 norm of (Phi_NN * F)
        num_inner = np.einsum("fm,fm->f", Phi_NN_F.conj(), Phi_NN_F)
        num = np.sqrt(np.real(num_inner) / M)
        
        # Denominator: F^H * Phi_NN * F
        den = np.einsum("fm,fm->f", F_gev.conj(), Phi_NN_F)
        
        # Calculate gain with small epsilon to prevent division by zero
        g_BAN = num / (np.real(den) + 1e-15)
        
        # Final weights
        weights = F_gev * g_BAN[:, np.newaxis]
...
def apply_hybrid_pipeline(time_domain_input, fs, model1_path, length_fft=512, hop_length_fft=128):
    ...
    print(" -> Running mask-based recursive GEV...")
    Y_stft = GEV_recursive_mask_based(X_stft, mask_s, mask_n)
