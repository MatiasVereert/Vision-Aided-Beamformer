...
# from beamforming.signal_model import compute_rtf_steering_vector
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, fs, array_geometry, source_pos, alpha=0.8, min_loading=1e-3, lamda=0.99, wiener_floor=0.05, save_weights=False):
    # Extract dimensions
    K, T, M = X_stft.shape
    ...
    for m in range(T):
        ...
        # Apply MVDR weights to get the intermediate clean output
        Y_mvdr = np.einsum("fm,fm->f", weights.conj(), X_frame)

        # --- DECOUPLED WIENER POST-FILTER (MWF formulation) ---
        # Calculate output speech power: w^H * R_ss * w
        w_Rss_w = np.einsum("fm,fmn,fn->f", weights.conj(), R_ss, weights)
        psd_s_out = np.maximum(np.real(w_Rss_w), 1e-12)

        # Calculate output noise power: w^H * R_nn * w
        w_Rnn_w = np.einsum("fm,fmn,fn->f", weights.conj(), Phi_NN_stable, weights)
        psd_n_out = np.maximum(np.real(w_Rnn_w), 1e-12)

        # Calculate Wiener gain: H_wf = PSD_s / (PSD_s + PSD_n)
        wiener_gain = psd_s_out / (psd_s_out + (mu * psd_n_out) + 1e-12)

        # Apply a floor to prevent extreme attenuation / musical noise
        wiener_gain = np.maximum(wiener_gain, wiener_floor)

        # Apply post-filter to the MVDR spatial output
        Y_stft[:, m] = wiener_gain * Y_mvdr
...
if __name__ == "__main__":
    ...
    output_folder = "tests/data/single_dtln_mvdr_mwf_output"
    ...
    output_rm, mask_rm = apply_hybrid_pipeline(
            ...
            length_fft=512,
            hop_length_fft=128,
            alpha=0.8 # Puedes ajustar cuánto confías en el Steering Vector vs el RTF empírico
    )
