...
# =====================================================================
# 1. ONLINE MVDR BEAMFORMER (MASK-BASED) WITH BSN
# =====================================================================
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, freqs, mic_coords, c=343.0, min_loading=1e-9, lamda=0.99, save_weights=False):
    K, T, M = X_stft.shape
    ...
    # =========================================================
    # PRE-COMPUTE STEERING VECTORS FOR BSN
    # =========================================================
    # Define an angular grid (e.g., 72 steps of 5 degrees for a full 2D circle)
    num_angles = 72
    azimuths = np.linspace(0, 2 * np.pi, num_angles, endpoint=False)

    # 3D Direction vectors assuming sources are on the horizontal plane (z=0)
    directions = np.array([np.cos(azimuths), np.sin(azimuths), np.zeros_like(azimuths)])

    # Time delays for each microphone from each angle: tau = (coords dot dir) / c
    # Shape of delays: (M, num_angles)
    delays = mic_coords @ directions / c

    # Steering vectors matrix D for all frequencies and angles
    # Shape of D_all: (K, M, num_angles)
    D_all = np.exp(-1j * 2 * np.pi * freqs[:, np.newaxis, np.newaxis] * delays[np.newaxis, :, :])
    # =========================================================
    ...
    for m in range(T):
        ...
        # Power Iteration: Matrix-vector multiplication using the previous d
        ...
        # =========================================================
        # BLIND STATISTICAL NORMALIZATION (BSN)
        # =========================================================
        # Calculate Spatial Response R(f, theta) = w^H * D
        # weights conj shape: (K, M)
        # D_all shape: (K, M, num_angles)
        # Resulting R shape: (K, num_angles)
        R = np.einsum("fm,fma->fa", weights.conj(), D_all)

        # Take the absolute magnitude of the spatial response
        R_abs = np.abs(R)

        # BSN Numerator: Sum of |R| over all evaluated angles
        bsn_num = np.sum(R_abs, axis=1)

        # BSN Denominator: Sum of |R|^2 over all evaluated angles
        bsn_den = np.sum(R_abs**2, axis=1)

        # Compute BSN scalar weights for the current frame
        w_bsn = bsn_num / (bsn_den + 1e-15)

        # Apply BSN scaling to the beamformer weights
        weights = weights * w_bsn[:, np.newaxis]
        # =========================================================
...
def apply_hybrid_pipeline(time_domain_input, fs, model1_path, ref_mic=None, length_fft=512, hop_length_fft=128):
    ...
    Y_stft = MVDR_recursive_mask_based(X_stft, mask_s, mask_n, freqs=freqs, mic_coords=mic_coords, min_loading=1e-9, lamda=0.99, save_weights=False)
...
if __name__ == "__main__":
    ...
    output_folder = "tests/data/hybrid_single_dtln_mvdr_output_bsn"
