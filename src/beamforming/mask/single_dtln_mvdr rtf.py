...
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, min_loading=1e-1, lamda=0.99, save_weights=False):
    # Extract dimensions
    K, T, M = X_stft.shape
    ...
    for m in range(T):
        ...
        # Replaces Power Iteration logic
        R_ss = Phi_XX
        ...
        d = np.where(update_mask[:, np.newaxis], rtf_empirical, d_prev)
