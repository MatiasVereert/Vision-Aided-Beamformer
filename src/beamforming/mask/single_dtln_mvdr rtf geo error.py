...
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, fs, array_geometry, source_pos, alpha=0.8, min_loading=1e-1, lamda=0.99, save_weights=False):
    # Extract dimensions
    K, T, M = X_stft.shape
    ...
    for m in range(T):
        ...
        # --- Direct RTF Estimation from Speech Covariance ---
        # Since Phi_XX is driven by the neural speech mask, it ALREADY approximates R_ss.
        R_ss = Phi_XX

        # Calculate empirical RTF relative to reference microphone (index 0)
        rtf_nom = R_ss[:, :, 0]
...
def apply_hybrid_pipeline(time_domain_input, fs, model1_path, array_geometry, source_pos, ref_mic=None, length_fft=512, hop_length_fft=128, alpha=0.8):
    ...
    print(" -> Running mask-based recursive MVDR with geometric anchor...")

    # NEW: Pass the geometric variables and alpha
    Y_stft = MVDR_recursive_mask_based(
...
if __name__ == "__main__":
    ...
    print("=== HYBRID TEST: DTLN MASK-BASED ONLINE MVDR WITH DOA ERROR ===")

    output_folder = "tests/data/single_dtln_mvdr_rtf_error_output"
    ...
    r_max = 0.30  # Maximum radius

    if M > 1:
        indices = np.arange(M)

        # Logarithmic radial expansion
        r_array = r_min * (r_max / r_min)**(indices / (M - 1))
        ...
        x = r_array * np.sin(phi) * np.cos(theta)
        y = r_array * np.sin(phi) * np.sin(theta)
        z = r_array * np.cos(phi)
    else:
    ...
    # =================================================================
    # TARGET AND INTERFERENCE POSITIONS (INTRODUCING DOA ERROR)
    # =================================================================
    r_dist = 1.0

    # True positions for the physical acoustic simulation
    ang_target_true = np.deg2rad(130)
    ang_interf = np.deg2rad(50)

    # Estimated position with a 5-degree error for the beamformer
    error_degrees = 5.0
    ang_target_estimated = np.deg2rad(130 + error_degrees)

    # Calculate 3D coordinates for true positions (Simulation)
    source_pos_true = array_center + np.array([r_dist * np.cos(ang_target_true), r_dist * np.sin(ang_target_true), 0.0])
    interf_pos1 = array_center + np.array([r_dist * np.cos(ang_interf), r_dist * np.sin(ang_interf), 0.0])
    source_pos_true_2d = source_pos_true.reshape(1, 3)

    # Calculate 3D coordinates for estimated position (Algorithm Input)
    estimated_source_pos = array_center + np.array([r_dist * np.cos(ang_target_estimated), r_dist * np.sin(ang_target_estimated), 0.0])
    estimated_source_pos_2d = estimated_source_pos.reshape(1, 3)

    print(" -> Initializing acoustic scene...")
    acoustic_scene = SimAcoustic(mic_coords, array_mismatch=0.0, duration=20, fs=FS)

    # Set the sound source at the TRUE physical location
    acoustic_scene.set_source(r"/home/matias/Documents/Tesis/Vision-Aided-Beamformer/tools/data/signals/p002_emo_adoration_sentences.wav", gain=1, position=source_pos_true_2d)
    ...
    # Pass the ESTIMATED geometry coordinates (with the 5-degree error) to the pipeline
    output_rm, mask_rm = apply_hybrid_pipeline(
        room_input,
        FS,
        MODEL_1_PATH,
        array_geometry=mic_coords,
        source_pos=estimated_source_pos_2d,  # Using the erroneous position as the anchor
        length_fft=512,
        hop_length_fft=128,
        alpha=0.8  # Balance between empirical RTF and this erroneous steering vector
    )

    save_wav("2_ROOM_hybrid_output_final_error_test.wav", FS, normalize_signal(output_rm), output_folder)
