...
def MVDR_recursive_mask_based(X_stft, mask_s, mask_n, min_loading=1e-9, mask_floor=1e-9, lamda=0.98, save_weights=False, ref_mic=0):
    K, T, M = X_stft.shape
    ...
    # Define the reference microphone vector (one-hot vector)
    r = np.zeros(M, dtype=np.complex128)
    r[ref_mic] = 1.0

    # Apply flooring to masks to prevent total cancellation of weak speech components
    ...
    for m in range(T):
        ...
        # Invert the stabilized noise covariance matrix
        Phi_NN_inv = np.linalg.inv(Phi_NN_stable)

        # Compute the product Phi_NN^-1 * Phi_XX
        matrix_product = np.matmul(Phi_NN_inv, Phi_XX)

        # Calculate the trace of the product matrix (normalization factor lambda)
        # Using np.real as the theoretical trace of this product is real
        trace_val = np.real(np.trace(matrix_product, axis1=1, axis2=2))

        # Direct MVDR formulation: weights = (Phi_NN^-1 * Phi_XX * r) / trace
        weights_nom = np.einsum("fmn,n->fm", matrix_product, r)
        weights = weights_nom / (trace_val[:, np.newaxis] + 1e-10)


        # =========================================================
        # BLIND ANALYTICAL NORMALIZATION (BAN)
        # =========================================================
        # Efficiently compute u = Phi_NN * weights
        u_ban = np.einsum("fmn,fn->fm", Phi_NN_stable, weights)

        # Compute denominator: w^H * Phi_NN * w = w^H * u
        ban_denom = np.real(np.einsum("fm,fm->f", weights.conj(), u_ban))

        # Compute numerator squared: w^H * Phi_NN * Phi_NN * w = u^H * u
        ban_num_sq = np.real(np.einsum("fm,fm->f", u_ban.conj(), u_ban))

        # Compute BAN scalar weights for the current frame
        w_ban = np.sqrt(ban_num_sq / M) / (ban_denom + 1e-15)

        # Apply BAN scaling to the beamformer weights
        weights = weights * w_ban[:, np.newaxis]
        # =========================================================

        if save_weights:
            weights_rec[:, m, :] = weights

        # Apply weights to the current frame to get the enhanced signal
...
if __name__ == "__main__":
    ...
    output_folder = "tests/data/hybrid_mvdr_frame-level_output"
