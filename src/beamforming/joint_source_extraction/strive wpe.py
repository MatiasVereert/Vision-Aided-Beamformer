...
if __name__ == "__main__":
    FS = 48000
    M1, M2 = 4, 1          # Arreglo lineal de 4 micrófonos
    ...
    acoustic_scene = SimAcoustic(mic_coords, array_mismatch=0.0, duration=5, fs=FS)
    ...
    dist_to_mics = np.linalg.norm(mic_coords - source_pos, axis=1)
    
    # INVERTIMOS EL SIGNO AQUÍ:
    tdoa = (dist_to_mics - dist_to_mics[0]) / speed_of_sound 

    for f_idx, freq in enumerate(freqs):
        for m in range(M):
            a_1[f_idx, m] = (1.0 / np.sqrt(M)) * np.exp(1j * 2 * np.pi * freq * tdoa[m])
