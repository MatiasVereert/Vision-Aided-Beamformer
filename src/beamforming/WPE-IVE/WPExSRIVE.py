    ...
    for Iter in range(N_Iter):
        ...
        for f in range(F):
            ...
            if frame_idx < M * 2: 
                ...
                # Initialize Pi_inv_1_f properly outside the time loop at t=0
                

                # --- EFFICIENT SPATIAL REGULARIZATION UPDATE ---
                # Calculate the cross term using the dereverberated signal
                y_f_contig = np.ascontiguousarray(y[f])

                # Extract the 2D matrix for the current frequency bin
                current_Pi_inv = Pi_inv_1_f[f]

                # Update using the Woodbury matrix identity
                Pi_inv_y = np.dot(current_Pi_inv, y_f_contig)
                y_H_Pi_inv = np.dot(np.conj(y_f_contig), current_Pi_inv)

                # Calculate scalar denominator
                denom_Pi = (alpha**2) * v_1 + alpha * (1.0 - alpha) * np.dot(y_H_Pi_inv, y_f_contig).real

                # Outer product for the numerator matrix
                numerator_matrix = np.outer(Pi_inv_y, y_H_Pi_inv) * (1.0 - alpha)

                # Efficiently update the global 3D array in-place without O(M^3) inversion
                Pi_inv_1_f[f] = (current_Pi_inv / alpha) - (numerator_matrix / denom_Pi)

                w_old = np.ascontiguousarray(W[f, :, 0])
                W_inv_e1 = np.ascontiguousarray(W_inv_H[f, :, 0])

                # Vectorwise Coordinate Descent (VCD) update for w_1 
                w_tmp = np.dot(Pi_inv_1_f[f], W_inv_e1)
                w_hat = lambda_unit * np.dot(Pi_inv_1_f[f], a_1[f])
...
def WPExSRIVE(x_stft, a_1, a_2, L=12, D=4):
    ...
    Pi_inv_1_f = np.zeros((F, M, M), dtype=np.complex128)
    ...
    for f in range(F):
        ...
        # Construct the spatial prior exactly as formulated in the paper
        spatial_prior = lambda_scale * np.eye(M, dtype=np.complex128) \
                        + lambda_unit * np.outer(a_1[f], np.conj(a_1[f])) \
                        + lambda_null * np.outer(a_2[f], np.conj(a_2[f]))
        Pi_inv_1_f[f] = np.linalg.inv(spatial_prior)

        # Initialize with a small noise floor to prevent division by zero
        for i in range(M):
            Sigma_1[f, i, i] = 1e-6
            Sigma_2[f, i, i] = 1e-6 
