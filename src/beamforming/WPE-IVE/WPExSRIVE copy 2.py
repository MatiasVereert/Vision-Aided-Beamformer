...
def process_frame(x_frame, x_bar, W, W_inv_H, G_1, G_2, 
    ...
    # Execute full block optimization inside the iteration loop
    for Iter in range(N_Iter):
        
        # 1. Recalculate dereverberated signals with current filters
        for f in range(F):
        ...
        # 2. Update variance based on current separation
        v_1 = 0.0
        for f in range(F):
            w_1_contig[:] = W[f, :, 0]
        ...
        # Floor to prevent explosion during silent frames
        v_1 = max(v_1, 1e-6) 
        
        # 3. Covariance, WPE, and VCD Updates
        for f in range(F):
            for i in range(M):
                for j in range(M):
            ...
            # WPE RLS Update (strictly executed only on the first iteration)
            if Iter == 0:
                # WPE RLS Update for Target
            ...
            # VCD Spatial Optimization
            if frame_idx >= M * 2: 
                # Exact spatial prior inversion computed per frequency bin
                for i in range(M):
                    for j in range(M):
                        Pi_1[i, j] = Sigma_1[f, i, j] + static_spatial_prior[f, i, j]
                Pi_inv_1_f = np.linalg.inv(Pi_1)
                
                w_old = np.ascontiguousarray(W[f, :, 0])
                W_inv_e1 = np.ascontiguousarray(W_inv_H[f, :, 0])

                # VCD Update for w_1 using the exact inverse
    ...
    for f in range(F):
        # Recalculate the separated source using the optimal converged filters
        y_1_final = x_frame[f] - np.dot(G_1[f].conj().T, x_bar[f])
        w_1_final = W[f, :, 0]
        s_1 = np.dot(w_1_final.conj(), y_1_final)
        
        # Scale back to reference mic
        s_hat[f] = W_inv_H[f, 0, 0] * s_1
...
def WPExSRIVE(x_stft, a_1, a_2, L=12, D=4):
    ...
    # Relaxed priors to act as anchors, not as fixed beamformers
    lambda_scale = 1e-3
    lambda_unit = 0.1
    lambda_null = 0.1
    alpha = 0.99
    ...
    # Pre-allocate the static spatial prior
    static_spatial_prior = np.zeros((F, M, M), dtype=np.complex128)

    for f in range(F):
        W[f] = np.eye(M, dtype=np.complex128)
        W_inv_H[f] = np.eye(M, dtype=np.complex128)
        R_inv_1[f] = 100.0 * np.eye(L * M, dtype=np.complex128)
        R_inv_2[f] = 100.0 * np.eye(L * M, dtype=np.complex128)

        # Pre-compute the static spatial prior entirely outside the time loop
        static_spatial_prior[f] = lambda_scale * np.eye(M, dtype=np.complex128) \
                                  + lambda_unit * np.outer(a_1[f], np.conj(a_1[f])) \
                                  + lambda_null * np.outer(a_2[f], np.conj(a_2[f]))
    ...
    for t in range(T):
        ...
        s_hat_frame = process_frame(
            ...
            Sigma_1, Sigma_2, R_inv_1, R_inv_2, 
            static_spatial_prior,  # Pass the pre-computed matrix
...
if __name__ == "__main__":
    ...
    # Assign proper distinct positions to the sources, not the array coordinates
    ...
    print(f" -> Average processing time per frame: {((end_time - start_time) / X_stft_ive.shape[1]) * 1000:.2f} ms")
