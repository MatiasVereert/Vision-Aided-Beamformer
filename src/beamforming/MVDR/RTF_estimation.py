...
def MVDR_recursive(X_stft, vad, fs, array_geometry, source_pos, length_fft, hop_length_fft, alpha=0.85):
    ...
    for m in range(T):
        ...
        # Isolate speech covariance matrix
        R_ss = R_xx - R_nn

        # Extract empirical RTF using microphone 0 as reference
        # Numerator: column 0 of R_ss for all frequencies, shape (K, M)
        rtf_nom = R_ss[:, :, 0]
        
        # Denominator: auto-power of reference mic, shape (K,)
        rtf_den = np.real(R_ss[:, 0, 0])
        
        # Ensure numerical stability to prevent division by zero or negative power
        rtf_den_safe = np.maximum(rtf_den, 1e-10)
        
        # Calculate empirical RTF vector, shape (K, M)
