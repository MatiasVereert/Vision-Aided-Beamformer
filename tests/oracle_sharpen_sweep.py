"""
oracle_sharpen_sweep.py
=======================
Barrido del exponente de realce (sharpen_exp) de la mascara ORACLE, para ver la
curva soft -> binario. Misma escena/config que oracle_mask_benchmark.py, pero:

  - Procesadores = Oracle a distintos sharpen_exp {1, 2, 3, 4, 6, 8} con alpha=1,
    mas el DTLN NM-MVDR alpha=1 como linea de referencia.
  - Se saltea el post-DTLN (interpreters=None): no aporta a este eje y acelera.

Resultados en ruta nueva:  tests/dataset_out/oracle_sharpen_sweep/

USO:
    conda activate tesis_beam
    python tests/oracle_sharpen_sweep.py
"""
...
OUTPUT_DIR = os.path.join(REPO_ROOT, "tests", "dataset_out", "oracle_sharpen_sweep")
...
def main():
    ...
    # base_config IDENTICO al del benchmark MIRD
    base_config = {
        'fs': 16000,
        'duration': 15,
        't_early': 0.050,
    ...
    # Procesadores: DTLN de referencia + Oracle barriendo el exponente
    processors_dict = {
        "DTLN_NM-MVDR_alpha_1": DTLN_MB_MVDR_SOUDEN_Processor(min_loading=1e-6, alpha=1),
    }
    for e in SHARPEN_EXPS:
        processors_dict[f"Oracle_exp{e:g}_alpha_1"] = ORACLE_MB_MVDR_SOUDEN_Processor(
            min_loading=1e-6, alpha=1, sharpen_exp=e
        )
    ...
    print("\n[Preview] Barrido sharpen_exp (Delta vs 'early'):")
