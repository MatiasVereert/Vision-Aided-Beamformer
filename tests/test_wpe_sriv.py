...
# Corrección: Importar el Frontend OLA requerido para inyectar la señal temporal
from beamforming.joint_source_extraction.wpe_srive import OnlineWPExSRIVE_N1_Vectorized, AcousticFrontend 
...
if __name__ == "__main__":
    ...
    acoustic_scene = SimAcoustic(mic_coords, array_mismatch=0.0, duration=12, fs = FS)
    ...
    # =========================================================================
    # PARTE D: EXTRACCIÓN CONJUNTA BSS+WPE (WPExSRIVE ONLINE)
    # =========================================================================
    print("\n--- [D] PROCESAMIENTO CONJUNTO BSS + WPE (ONLINE WPExSRIVE) ---")
    
    # Adaptación paramétrica a Fs=48kHz
    n_fft_joint = 1024
    hop_length_joint = 512

    print(f"  -> Inicializando AcousticFrontend (M={M}, N_FFT={n_fft_joint})...")
    frontend_joint = AcousticFrontend(
        M=M, fs=FS, n_fft=n_fft_joint, hop_length=hop_length_joint, 
        Rs=target_pos_flat, mic_array=mic_coords
    )

    n_samples_room = room_input_mic.shape[1]
    out_room_joint_mic = np.zeros(n_samples_room)

    print(f"  -> Ejecutando extracción conjunta iterativamente en {n_samples_room} muestras...")
    for i in range(0, n_samples_room - hop_length_joint, hop_length_joint):
        chunk_in = room_input_mic[:, i:i+hop_length_joint]
        out_room_joint_mic[i:i+hop_length_joint] = frontend_joint.process_chunk(chunk_in)

    save_wav("9_output_room_JOINT_WPExSRIVE_MIC.wav", FS, out_room_joint_mic, output_folder)
