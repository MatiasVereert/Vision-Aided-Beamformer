...
if __name__ == "__main__":
    print("=== INTEGRATION TEST: WPE & GWPE ONLY ===")
    ...
    save_wav("2_input_room_MIC.wav", FS, room_input_mic[0], output_folder)

    # =========================================================================
    # PARTE B: DEREVERBERACIÓN (WPE) 
    # =========================================================================
    print("\n--- [B] PROCESAMIENTO CON DEREVERBERACIÓN (WPE) ---")

    # --- B.1 Caso Micrófonos Reales (WPE) ---
    print("  -> [MIC] Aplicando WPE a 'room_input_mic'...")
    room_input_wpe_mic = apply_wpe(room_input_mic, FS, taps=10, delay=3, iterations=3)
    save_wav("3a_output_room_WPE_MIC.wav", FS, room_input_wpe_mic[0], output_folder)

    # --- B.2 Caso Micrófonos Ideales (WPE) ---
    ...
    save_wav("3b_output_room_WPE_IDEAL.wav", FS, room_input_wpe_ideal[0], output_folder)

    # =========================================================================
    # PARTE C: DEREVERBERACIÓN (GWPE)
    # =========================================================================
    print("\n--- [C] PROCESAMIENTO CON DEREVERBERACIÓN (GWPE) ---")

    # --- C.1 Caso Micrófonos Reales (GWPE) ---
    print("  -> [MIC] Aplicando GWPE a 'room_input_mic'...")
    room_input_gwpe_mic = batch_dereverb(room_input_mic, FS, K=K_GWPE)
    save_wav("4a_output_room_GWPE_MIC.wav", FS, room_input_gwpe_mic[0], output_folder)

    # --- C.2 Caso Micrófonos Ideales (GWPE) ---
    print("  -> [IDEAL] Aplicando GWPE a 'room_input_ideal'...")
    room_input_gwpe_ideal = batch_dereverb(room_input_ideal, FS, K=K_GWPE)
    save_wav("4b_output_room_GWPE_IDEAL.wav", FS, room_input_gwpe_ideal[0], output_folder)
